...
def add_arguments(args):
    ...
    args.add_argument('--min_lr', type=float, help="min lr", default=1e-5)
    ...
    args.add_argument('--pooling', type=str, help="pooing_type", choices=['memory', 'avg', 'min', 'max', 'hierarchical', 'attention'], default = "memory")
    ...
    # dataset graph paras
    args.add_argument('--usecoo', help="use co-organization edge", action='store_true', default = False)
    args.add_argument('--usecov', help="use co-venue edge", action='store_true', default = False)
    args.add_argument('--threshold', type=float, help="threshold of coo and cov", default=0.1)
...
if __name__ == "__main__":
    ...
    encoder = GraphCAD(logger, args, args.input_dim, args.out_dim, args.outer_layer, args.inner_layer, is_global = False, is_edge = args.is_edge, pooling= args.pooling).cuda()
    ...
    for epoch_num in range(args.epochs):
        ...
        for tmp_train in tqdm(train_data):
            ...
            adj_weight = adj_weight.cpu()
            # print("##############################")
            # print(adj_weight.shape)
            if args.threshold > 0:
            ...
            if args.usecoo and args.usecov:
                ...
                # 选择最大值
                # tensor([0.3, 0.6, 0.9])
                adj_weight = torch.max(adj_weight, dim=-1).values
            elif args.usecoo:
