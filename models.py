...
class GraphCAD(nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight, batch_item, bs):
        ...
        for index in range(self.total_layer_num):
            # edge update
            if self.is_edge:
                # edge_index, edge_weight, edge_prob, x_trans_loss
                edge_index, edge_weight, edge_prob, _ = self.edgemodel(x, edge_index, edge_weight, centroid, batch_item, bs)
            else:
...
class SystemUpdate(nn.Module):
    ...
    def forward(self, his_x, init_c, bs):
        ...
        if self.pooling == "memory":
            ...
            init_c = torch.zeros(bs, self.in_dim, device=his_x[0].device)
            init_c_list = []
            for x in his_x:
                x = x.view(bs, -1, self.in_dim)
                init_c_list.append(torch.mean(x, dim = 1))
            # sort the list
            init_c_list = sorted(init_c_list, key=lambda x: x[0][0], reverse=True)
            return init_c_list[len(init_c_list) / 2]
        elif self.pooling == "attention":
            # 新增 attention 池化方式
            x = his_x[-1].view(bs, -1, self.in_dim)
            attn_weights = F.softmax(torch.bmm(x, init_c.view(bs, self.in_dim, 1)).view(bs, -1), dim=1)
            init_c = torch.sum(attn_weights.view(bs, -1, 1) * x, dim=1)
        else:
