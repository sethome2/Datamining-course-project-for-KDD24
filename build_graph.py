...
def chinese2pinyin(string):
    pinyin_text = ' '.join(lazy_pinyin(string))
    return pinyin_text

from transformers import MarianMTModel, MarianTokenizer

local_model_path = './opus-mt-zh-en'
tokenizer = MarianTokenizer.from_pretrained(local_model_path)
model = MarianMTModel.from_pretrained(local_model_path)
model = model.to('cuda')
translate_cache = {}
def chinese2english(src_text):
    global tokenizer
    global model
    global translate_cache
    
    if src_text[0] in translate_cache:
        return translate_cache[src_text[0]]
    
    # 对文本进行编码
    encoded = tokenizer(src_text, return_tensors='pt', padding=True)
    encoded = {key: val.to('cuda') for key, val in encoded.items()}  # 将输入移动到 GPU 上
    ...
    translate_cache[src_text[0]] = translated_text[0]
    print(f"Translating {src_text[0]} to {translated_text[0]}")
...
def init_worker(names, info, emb):
    global author_names
    global papers_info
    global dic_paper_embedding
    
    author_names = names
    papers_info = info
    dic_paper_embedding = emb
...
def build_dataset(path):
    ...
    with mp.Pool(processes=8, initializer=init_worker, 
                 initargs=(author_names, papers_info, dic_paper_embedding)) as pool:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_data_dir', type=str, default='dataset/pid_to_info_all_clean.json')
    ...
    # 遍历所有的pub中的venue, 如果venue中包含中文，则翻译为英文
    for k,v in papers_info.items():
        if is_contains_chinese(v['venue']):
            papers_info[k]['venue'] = chinese2english([v['venue']])
    # save memory
    del translate_cache
    del tokenizer
    del model
    # save cleaned pub
    with open(args.save_data_dir, "w", encoding = "utf-8") as f:
        js.dump(papers_info, f)
